...
describe 'A registered user', :js do
  ...
  it 'user bookmarks are grouped by tutorial and displayed in proper sequence' do
    ...
    within('.bookmarked-segments') do
      ...
      expect(page.all('.bookmarked-tutorial')[0]).to have_content(tutorial1.title.to_s)
      expect(page.all('.bookmarked-tutorial')[0]).to have_css('.bookmarked-video', count: 2)
      expect(page.all('.bookmarked-video')[0]).to have_content(video1.title.to_s)
      expect(page.all('.bookmarked-video')[1]).to have_content(video2.title.to_s)

      expect(page.all('.bookmarked-tutorial')[1]).to have_content(tutorial3.title.to_s)
      expect(page.all('.bookmarked-tutorial')[1]).to have_css('.bookmarked-video', count: 1)
      expect(page.all('.bookmarked-video')[2]).to have_content(video4.title.to_s)

      expect(page).to_not have_content(tutorial2.title.to_s)
      expect(page).to_not have_content(video3.title.to_s)
    end
  end
end
